...
// Loads the ICD and its WGL extensions
//
static void loadExtensions(void)
{
    HGLRC rc;

    // Create dummy OpenGL context to force loading of the OpenGL ICD
    //
    // The ICD (Installable Client Driver) must be loaded before we can query
    // for WGL extensions, including extensions related to context creation
    // 
    // Yes, this is strange, and yes, this is how WGL works
    // 
    // As Windows only allows you to set the pixel format once for a window, we
    // create a dummy context on the helper window to avoid having to re-create
    // windows later
    {
        PIXELFORMATDESCRIPTOR pfd;
        HDC dc = GetDC(_glfw.win32.helperWindow);
        ...
        {
            _glfwInputError(GLFW_FORMAT_UNAVAILABLE,
                            "WGL: Failed to find pixel format for dummy OpenGL context");
            return;
        }
        ...
        {
            _glfwInputError(GLFW_API_UNAVAILABLE,
                            "WGL: Failed to create dummy OpenGL context");
            return;
        }
        ...
        if (!_glfw.wgl.MakeCurrent(dc, rc))
        {
            ...
            _glfwInputError(GLFW_PLATFORM_ERROR,
                            "WGL: Failed to make dummy OpenGL context current");
            return;
        }
    }
                       ...
                       const _GLFWfbconfig* fbconfig)
{
    ...
    if (_glfw.wgl.ARB_create_context)
    {
            ...
            if (ctxconfig->profile)
            {
                if (!_glfw.wgl.ARB_create_context_profile)
                {
                    _glfwInputError(GLFW_VERSION_UNAVAILABLE,
                                    "WGL: An OpenGL profile requested but WGL_ARB_create_context_profile is unavailable");
                    return GLFW_FALSE;
                }
            }
        ...
        {
            if (!_glfw.wgl.EXT_create_context_es2_profile)
            {
                _glfwInputError(GLFW_API_UNAVAILABLE,
                                "WGL: OpenGL ES requested but WGL_EXT_create_context_es2_profile is unavailable");
                return GLFW_FALSE;
            }
    ...
    else
    {
        ...
        window->context.wgl.handle =
            _glfw.wgl.CreateContext(window->context.wgl.dc);
        if (!window->context.wgl.handle)
        {
            _glfwInputError(GLFW_VERSION_UNAVAILABLE,
                            "WGL: Failed to create OpenGL context");
...
void _glfwPlatformMakeContextCurrent(_GLFWwindow* window)
{
    if (window)
    {
        _glfw.wgl.MakeCurrent(window->context.wgl.dc,
                              window->context.wgl.handle);
    }
    else
        _glfw.wgl.MakeCurrent(NULL, NULL);
